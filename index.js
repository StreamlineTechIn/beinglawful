...
// GET route - render buyBook page
app.get("/buy-book", (req, res) => {
  res.render("buyBook", { error: null });
});

// POST route - handle form submission
app.post("/submit-book-order", async (req, res) => {
  try {
    const {
      buyerName,
      email,
    ...
    await db.collection("bookOrders").add({
      buyerName,
      email,
      phone,
      address,
    ...
    // Redirect or render a success page (you can create one)
    res.redirect("/buy-book?success=true");
  } catch (err) {
    console.error("Error saving order:", err);
    res.render("buyBook", { error: "Something went wrong. Please try again." });
  }
});
